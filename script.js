...
// JavaScript to fix navbar only during Slider 1
const navbar = document.querySelector('.navbar');
const slideshowHeight = document.querySelector('.slideshow').offsetHeight;

window.addEventListener('scroll', () => {
    if (window.scrollY < slideshowHeight) {
        navbar.style.position = 'fixed'; // Fix navbar during Slider 1
    } else {
        navbar.style.position = 'absolute'; // Set navbar to absolute for other sections
    }
});
